...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { CzlInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw CzlInvalidDataException("`$name` is not set")
            is JsonNull -> throw CzlInvalidDataException("`$name` is null")
            else -> throw CzlInvalidDataException("`$name` is invalid, received $this")
        }
